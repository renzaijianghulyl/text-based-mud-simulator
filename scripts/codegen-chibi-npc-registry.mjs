/**
 * 从 scenarios/chibi 读取 config + npcs/*.json，生成 src/sessions/chibi-npc-registry.generated.ts
 */
...
const scenarioDir = join(root, 'scenarios', 'chibi');
const configPath = join(scenarioDir, 'config.json');
const outPath = join(root, 'src', 'sessions', 'chibi-npc-registry.generated.ts');

const config = JSON.parse(readFileSync(configPath, 'utf-8'));
const availableNpcs = Array.isArray(config.availableNpcs) ? [...config.availableNpcs] : [];
const defaultTarget =
  typeof config.defaultTarget === 'string' && config.defaultTarget ? config.defaultTarget : 'zhou-yu';
...
if (ids.length === 0) {
  throw new Error('codegen chibi: no NPC templates resolved');
}
...
const body = `
export const CHIBI_DEFAULT_TARGET_NPC_ID = ${JSON.stringify(defaultTarget)} as const;

export const CHIBI_AVAILABLE_NPC_IDS = ${JSON.stringify(ids)} as const;

const RAW_TEMPLATES = ${jsonLiteral} as Record<string, CurrentNpc>;

export const CHIBI_REL_INITIAL = ${relLiteral} as Readonly<Record<string, Readonly<Record<string, number>>>>;
